...
const OperationAuthenticationServiceCurrentUserInfo = "/admin.service.v1.AuthenticationService/CurrentUserInfo"
const OperationAuthenticationServiceCurrentUserMenus = "/admin.service.v1.AuthenticationService/CurrentUserMenus"
...
type AuthenticationServiceHTTPServer interface {
	...
	// CurrentUserMenus 当前用户的菜单（根据用户角色返回绑定的菜单树）
...
func RegisterAuthenticationServiceHTTPServer(s *http.Server, srv AuthenticationServiceHTTPServer) {
	...
	r.GET("admin/v1/current-user", _AuthenticationService_CurrentUserInfo0_HTTP_Handler(srv))
	r.GET("/admin/v1/current-user/menus", _AuthenticationService_CurrentUserMenus0_HTTP_Handler(srv))
}
...
func _AuthenticationService_CurrentUserInfo0_HTTP_Handler(srv AuthenticationServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in emptypb.Empty
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationAuthenticationServiceCurrentUserInfo)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
...
func _AuthenticationService_CurrentUserMenus0_HTTP_Handler(srv AuthenticationServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in emptypb.Empty
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationAuthenticationServiceCurrentUserMenus)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.CurrentUserMenus(ctx, req.(*emptypb.Empty))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*v1.CurrentUserMenusResponse)
		return ctx.Result(200, reply)
	}
}
...
type AuthenticationServiceHTTPClient interface {
	// CurrentUserInfo 当前用户
	CurrentUserInfo(ctx context.Context, req *emptypb.Empty, opts ...http.CallOption) (rsp *v1.CurrentUserInfoResponse, err error)
	// CurrentUserMenus 当前用户的菜单（根据用户角色返回绑定的菜单树）
	CurrentUserMenus(ctx context.Context, req *emptypb.Empty, opts ...http.CallOption) (rsp *v1.CurrentUserMenusResponse, err error)
	// GetCaptcha 获取验证码
...
// CurrentUserInfo 当前用户
func (c *AuthenticationServiceHTTPClientImpl) CurrentUserInfo(ctx context.Context, in *emptypb.Empty, opts ...http.CallOption) (*v1.CurrentUserInfoResponse, error) {
	var out v1.CurrentUserInfoResponse
	pattern := "admin/v1/current-user"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationAuthenticationServiceCurrentUserInfo))
...
// CurrentUserMenus 当前用户的菜单（根据用户角色返回绑定的菜单树）
func (c *AuthenticationServiceHTTPClientImpl) CurrentUserMenus(ctx context.Context, in *emptypb.Empty, opts ...http.CallOption) (*v1.CurrentUserMenusResponse, error) {
	var out v1.CurrentUserMenusResponse
	pattern := "/admin/v1/current-user/menus"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationAuthenticationServiceCurrentUserMenus))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
