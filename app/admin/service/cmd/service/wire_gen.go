...
func wireApp(confServer *conf.Server, confData *conf.Data, jwt *conf.JWT, logger log.Logger) (*kratos.App, func(), error) {
	...
	organizationService := service.NewOrganizationService(organizationUsecase, positionUsecase)
	grpcServer := server.NewGRPCServer(confServer, authenticationService, permissionService, organizationService, logger)
	httpServer := server.NewHTTPServer(confServer, jwt, authenticationService, permissionService, organizationService, logger)
